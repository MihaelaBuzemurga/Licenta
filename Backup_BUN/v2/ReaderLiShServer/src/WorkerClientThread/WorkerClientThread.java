		...
		while (true) {
			...
			try {
				...
				if (initilize[0] == 2) {
					...
					System.out.println(recv_data.length+4);
					Message message=new Message(recv_data);
					RequestEnum cmd = RequestEnum.fromId(Integer.parseInt(new String(cmd_buff)));
					switch (cmd) {
					case REQUEST_LOGIN:
					...
					case REQUEST_UPLOAD_FILE:
						FileManager.getInstance().uploadFile(message);
						break;
					case REQUEST_EDIT_FILE:
						FileManager.getInstance().uploadFile(message);
						break;
					case REQUEST_FILE:
						mesaj=FileManager.getInstance().getFile(message);
						dout.write(mesaj.getDataPacket("101".getBytes("UTF8")));
						dout.flush();
						break;
					default:
