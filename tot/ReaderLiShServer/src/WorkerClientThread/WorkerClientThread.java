...
import Reusable.*;
import SentiManager.SentiManager;
				...
				if (initilize[0] == 2) {
					...
					if (recv_data != null) {
						...
						case REQUEST_ADD_NOTA:
							newMesaj=FileManager.getInstance().addNota((Carte) m_mesaj.getObiect());
							 dout.write(TransformerBytes.getDataPacket(newMesaj));
							 dout.flush();
							 break;
						case REQUEST_GEN:
							newMesaj=FileManager.getInstance().requestGenuri();
							 ...
							 break;
						case REQUEST_SENTI:
							newMesaj=SentiManager.getSession().getSenti((String) m_mesaj.getObiect());
							 dout.write(TransformerBytes.getDataPacket(newMesaj));
							 dout.flush();
							 break;
						case REQUEST_CHECK_NOTIFICATION:
							newMesaj=SessionManager.getSession().getNotification((int) m_mesaj.getObiect());
							 dout.write(TransformerBytes.getDataPacket(newMesaj));
							 dout.flush();
							 break;
						default:
